...
void ANZP_PlayerCharacter::DropPointsFunction(const FInputActionValue& Value)
{
	...
	{
		//TODO: Drop points.
		GetWorld()->SpawnActor<ANZP_Drops_ZombiePoints>(Anzp_Drops_ZombiePointsReference,
...
void ANZP_PlayerCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* PlayerEnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		if (Fire)
		{
			PlayerEnhancedInputComponent->BindAction(Fire, ETriggerEvent::Triggered, this,
			                                         &ANZP_PlayerCharacter::Firing);
		}
		if (Looking)
		{
			PlayerEnhancedInputComponent->BindAction(Looking, ETriggerEvent::Triggered, this,
			                                         &ANZP_PlayerCharacter::LookingAround);
		}
		if (MovementAction)
		{
			PlayerEnhancedInputComponent->BindAction(MovementAction, ETriggerEvent::Triggered, this,
			                                         &ANZP_PlayerCharacter::Movement);
		}

		if (CrouchAction)
		{
			PlayerEnhancedInputComponent->BindAction(CrouchAction, ETriggerEvent::Triggered, this,
			                                         &ANZP_PlayerCharacter::Crouching);
		}

		if (DropPoints)
		{
			PlayerEnhancedInputComponent->BindAction(DropPoints, ETriggerEvent::Completed, this,
			                                         &ANZP_PlayerCharacter::DropPointsFunction);
		}
		if (InteractionAction)
		{
			PlayerEnhancedInputComponent->BindAction(InteractionAction, ETriggerEvent::Triggered, this,
			                                         &ANZP_PlayerCharacter::Interaction);
		}
	}
}
